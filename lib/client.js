...
/*
 * Override the default Meteor._relativeToSiteRootUrl()
 *
 */
override(Meteor, '_relativeToSiteRootUrl', function(original) {
  return function(link) {
    var CDN_URL = __meteor_runtime_config__.CDN_URL || Meteor.settings?.packages?.["storyteller:cdn"]?.CDN_URL;
...
CDN.get_cdn_url = function(){
  return __meteor_runtime_config__.CDN_URL  || Meteor.settings?.packages?.["storyteller:cdn"]?.CDN_URL || "";
}
